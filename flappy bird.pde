...
float maybeRandomHeight() {
  if (space%50==0) {  //changing the speed was key // creates even spacing based the fact that it equals zero
...
void draw() {
  ...
  if (mousePressed) {
    ellipse (30, y, 30, 30);
    y= y - 8; //this is the part that makes the circle go up subtract
  }
